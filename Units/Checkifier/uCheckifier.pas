...
type
  ...
  private
    FUsingEmbeddedChecksum: Boolean;
    FFirstLineCommentCaption: string;
...
procedure TCheckifier.About1Click(Sender: TObject);
begin
  ShellAbout(Self.Handle, '', MY_APP_NAME_CHECKIFY + ' v'+ MY_APP_VERSION  +slineBreak+'Developer: '+ MY_APP_DEVELOPER,
  Application.Icon.Handle);
end;

procedure TCheckifier.Browsethefile1Click(Sender: TObject);
var
  Item: TListItem;
  FullPath: string;
begin
...
procedure TCheckifier.Cancel1Click(Sender: TObject);
begin
  PopupMenu1.CloseMenu;
end;

procedure TCheckifier.CancledByUser;
begin
...
procedure TCheckifier.CloseAllAlgHandles;
begin
...
procedure TCheckifier.CloseCurrent1Click(Sender: TObject);
begin
  if FScanning then
    Exit;
  ...
  FUsingEmbeddedChecksum := False;
  FFirstLineCommentCaption := '';
...
procedure TCheckifier.CopyFIleName1Click(Sender: TObject);
begin
  if Assigned(QCListView1.Selected) then
    Clipboard.AsText := QCListView1.Selected.SubItems[0];
end;

procedure TCheckifier.CopyFileName2Click(Sender: TObject);
begin
  if Assigned(QCListView1.Selected) then
    Clipboard.AsText := QCListView1.Selected.SubItems[0];
end;

procedure TCheckifier.Exit1Click(Sender: TObject);
begin
  if FScanning then
  begin
    FCancelRequested := True;
...
procedure TCheckifier.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if FScanning then
  begin
    FCancelRequested := True;
...
procedure TCheckifier.FormCreate(Sender: TObject);
var
  ChecksumOpenWithQChecikfy: string; TempLines: TStringList;
begin
//    Clipboard.AsText:=CalculateUniversalResourceHash('TCheckifier', RT_RCDATA);
  ...
  if EmbeddedChecksumResourceExists then
  begin
    TempLines := TStringList.Create;
    try
      if LoadLinesFromEmbeddedChecksum(TempLines) and HasValidChecksumLines(TempLines) then
      begin
        FUsingEmbeddedChecksum := True;
        Self.Caption := MY_APP_NAME_CHECKIFY;
        QCMDHashChecksmProc(IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0))));
      end;
    finally
      TempLines.Free;
    end;
...
procedure TCheckifier.FormDestroyHandler(Sender: TObject);
begin
  FreeQCListViewItemsData;
  SavingQCLastChanges;
  if Assigned(FAllItems) then
...
procedure TCheckifier.FormResizeFixQCColumn(Sender: TObject);
begin
  ColumnsStretchtoForm;
end;

procedure TCheckifier.FreeQCListViewItemsData;
var
  i: Integer;
begin
...
procedure TCheckifier.ColumnsStretchtoForm;
var
...
procedure TCheckifier.GettingReadyQCListView;
var
  Header: HWND;
  Style: Integer;
begin
...
procedure TCheckifier.LoadChecksum1Click(Sender: TObject);
begin
  if FScanning then
    Exit;
...
function TCheckifier.GetMergedSubItemRect(ItemIndex, FromSubItem, ToSubItem: Integer): TRect;
var
  i: Integer;
...
function TCheckifier.GetSharedAlgHandle(Kind: TQCHashKind; out DigestLen: Cardinal): Pointer;
begin
  case Kind of
    qcMD5:
      begin
...
procedure TCheckifier.ListViewResult(Sender: TCustomListView;
...
procedure TCheckifier.QCListView1CustomDrawItem(Sender: TCustomListView;
  Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);
var
...
procedure TCheckifier.QCListView1Changing(Sender: TObject; Item: TListItem;
  Change: TItemChange; var AllowChange: Boolean);
begin
...
procedure TCheckifier.QCListView1MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  Item: TListItem;
  FilePath: string;
...
procedure TCheckifier.FixListViewSize(Sender: TObject);
begin
  ColumnsStretchtoForm;
end;

procedure TCheckifier.PushProgressUI(FilePerc: Integer; OverallPerc: Double; ForceUpdate: Boolean);
var
  Now_: Cardinal;
begin
  Now_ := GetTickCount;
...
function TCheckifier.QCCalculateCRC32WithProgress(const FileName: string): string;
const
...
function TCheckifier.QCCalculateMD5WithProgress(const FileName: string): string;
const
...
function TCheckifier.QCCalculateSHA1WithProgress(const FileName: string): string;
const
  MaxBufferSize = 16 * 1024 * 1024;
var
  Stream: THandleStream;
...
function TCheckifier.QCCalculateSHA256WithProgress(const FileName: string): string;
const
  MaxBufferSize = 16 * 1024 * 1024;
var
  Stream: THandleStream;
...
function TCheckifier.QCCalculateSHA384WithProgress(const FileName: string): string;
const
  MaxBufferSize = 16 * 1024 * 1024;
var
  Stream: THandleStream;
...
function TCheckifier.QCCalculateSHA512WithProgress(const FileName: string): string;
const
  MaxBufferSize = 16 * 1024 * 1024;
var
  Stream: THandleStream;
...
procedure TCheckifier.QCMDHashChecksmProc(const HashChecksumFile: string; Resuming: Boolean = False);
begin
  if FScanning then
    Exit;
    ...
    begin
      try
        ...
        begin
          ...
          try
             if FUsingEmbeddedChecksum then
            begin
              if not LoadLinesFromEmbeddedChecksum(FLines) then
                raise Exception.Create('Embedded checksum resource not found or could not be read.');
            end
            else
            ...
            begin
              ...
              TThread.Synchronize(nil,
                ...
                begin
                  GroupBox1.Visible := False;
                  // এমবেডেড রিসোর্স damaged/missing হলে কোনো মেসেজ দেখানো হবে না — silently ignore
                  if not FUsingEmbeddedChecksum then
         ...
         // ভ্যালিড হ্যাশ লাইন না থাকলে স্ক্যান শুরু করব না (silent, কোনো message নেই)
          if not HasValidChecksumLines(FLines) then
          begin
            TThread.Synchronize(nil,
              procedure
              begin
          ...
          FFirstLineCommentCaption := ExtractFirstLineCommentCaption(FLines);
          ...
          TThread.Synchronize(nil,
            ...
            if FFirstLineCommentCaption <> '' then
                Caption := FFirstLineCommentCaption;
            end);
        ...
        TThread.Synchronize(nil,
          ...
          //  Caption := HashChecksumFile;
            if FFirstLineCommentCaption <> '' then
              Caption := FFirstLineCommentCaption
            else if FUsingEmbeddedChecksum then
              Caption := MY_APP_NAME_CHECKIFY
            else
              Caption := HashChecksumFile;
            if not FCancelRequested then
...
procedure TCheckifier.ReScanChecksumFile;
begin
   if FUsingEmbeddedChecksum then
  begin
    FCancelRequested := False;
...
procedure TCheckifier.RestartScan1Click(Sender: TObject);
begin
...
procedure TCheckifier.DisableEverythingProcesStarted;
begin
...
procedure TCheckifier.EnableEverythingProcesEnded;
var
  HasItems: Boolean;
begin
  StopTasks1.Enabled := False;
...
procedure TCheckifier.SavingQCLastChanges;
begin
...
procedure TCheckifier.ContinueScan1Click(Sender: TObject);
begin
...
procedure TCheckifier.SetFilterToogle(AItem: TMenuItem);
begin
...
procedure TCheckifier.ApplyFilter;
var
  i: Integer;
  Rec: TQCFileRecord;
  Item: TListItem;
...
procedure TCheckifier.ShowAll1Click(Sender: TObject);
begin
  SetFilterToogle(ShowAll1);
  ApplyFilter;
end;

procedure TCheckifier.ShowGenuineFileOnly1Click(Sender: TObject);
begin
  SetFilterToogle(ShowGenuineFileOnly1);
  ApplyFilter;
end;

procedure TCheckifier.ShowMismatchedFileOnly1Click(Sender: TObject);
begin
  SetFilterToogle(ShowMismatchedFileOnly1);
  ApplyFilter;
end;

procedure TCheckifier.ShowMissingOnly1Click(Sender: TObject);
begin
...
procedure TCheckifier.StartingQCFileChecsum(MDChecksumPath: string);
begin
  QCMDHashChecksmProc(MDChecksumPath);
end;

procedure TCheckifier.StopTasks1Click(Sender: TObject);
begin
  CancledByUser;
    TaskbarControll.ProgressState:=TTaskbarProgressState.Error;
end;

procedure TCheckifier.WMDropFiles(var Msg: TWMDropFiles);
var
